...
class Spreadsheet_Import extends Spreadsheet_Excel_Reader{
	function import_data($sheet=0) {
		
		$node = new stdClass();
		$node->type = 'sales_result';
		$node->language ='';
		$node->uid = 1;
		$node->name = "superadmin";
		$node->status = 1;
		$node->revision = 0;
		...
		$totalField = count($arrF);
		$totalRow = $this->rowcount($sheet);
		
		for($row=2;$row<=$totalRow;$row++) {
			$node->nid = NULL;
			$node->vid = NULL;
			$node->title = $this->val($row,3,$sheet);
			//$node->body = $this->val($row,3,$sheet);
			$node->field_sale_name[0]['value'] = $this->val($row,3,$sheet);
			$arrD = split('/',$this->val($row,2,$sheet));
			if(count($arrD)==3)
				$node->field_sale_date[0]['value'] = $arrD[2].'-'.$arrD[0].'-'.$arrD[1]; 
			else
				$node->field_sale_date[0]['value'] = date('Y-m-d');
			
			
			$node->field_sale_city[0]['value'] = $this->val($row,4,$sheet);
			$node->field_sale_state[0]['value'] = $this->val($row,5,$sheet);
			$node->field_sale_region[0]['value'] = $this->val($row,6,$sheet);
			$node->field_sale_overall_lots[0]['value'] = $this->getNumber($this->val($row,10,$sheet));
			$node->field_sale_overall_avg[0]['value'] = $this->getNumber($this->val($row,12,$sheet));
			$node->field_sale_overall_gross[0]['value']  = $this->getNumber($this->val($row,11,$sheet));
			//$node->field_sale_overall_top[0]['value']  = $this->val($row,16,$sheet);
			$node->field_sale_bull_lots[0]['value']  = $this->getNumber($this->val($row,13,$sheet));
			$node->field_sale_bull_avg[0]['value'] = $this->getNumber($this->val($row,15,$sheet));
			$node->field_sale_bull_gross[0]['value'] = $this->getNumber($this->val($row,14 ,$sheet));
			$node->field_sale_bull_top[0]['value'] = $this->getNumber($this->val($row,16 ,$sheet));
			$node->field_sale_female_lots[0]['value'] = $this->getNumber($this->val($row,18 ,$sheet));
			$node->field_sale_female_avg[0]['value'] = $this->getNumber($this->val($row,20 ,$sheet));
			$node->field_sale_female_gross[0]['value'] = $this->getNumber($this->val($row,19 ,$sheet));
			$node->field_sale_female_top[0]['value'] = $this->getNumber($this->val($row,21 ,$sheet));
			$node->field_sale_flush_lots[0]['value'] = $this->getNumber($this->val($row,23 ,$sheet));
			$node->field_sale_flush_avg[0]['value'] = $this->getNumber($this->val($row,25 ,$sheet));
			$node->field_sale_flush_gross[0]['value'] = $this->getNumber($this->val($row,24 ,$sheet));
			$node->field_sale_flush_top[0]['value'] = $this->getNumber($this->val($row,26 ,$sheet));
			$node->field_sale_embryo_lots[0]['value'] = $this->getNumber($this->val($row,28 ,$sheet));
			$node->field_sale_embryo_avg[0]['value'] = $this->getNumber($this->val($row,30 ,$sheet));
			$node->field_sale_embryo_gross[0]['value'] = $this->getNumber($this->val($row,29 ,$sheet));
			$node->field_sale_embryo_top[0]['value'] = $this->getNumber($this->val($row,31 ,$sheet));
			$node->field_sale_comm_lots[0]['value'] = $this->getNumber($this->val($row,33 ,$sheet));
			$node->field_sale_comm_avg[0]['value'] = $this->getNumber($this->val($row,35 ,$sheet));
			$node->field_sale_comm_gross[0]['value'] = $this->getNumber($this->val($row,34 ,$sheet));
			$node->field_sale_comm_top[0]['value'] = $this->getNumber($this->val($row,36 ,$sheet));
			$node->field_sale_calf_avg[0]['value'] = $this->getNumber($this->val($row,40 ,$sheet));
			$node->field_sale_calf_gross[0]['value'] = $this->getNumber($this->val($row,39 ,$sheet));
			$node->field_sale_calf_top[0]['value'] = $this->getNumber($this->val($row,41 ,$sheet));
			$node->field_sale_calf_lots[0]['value'] = $this->getNumber($this->val($row,38 ,$sheet));
			$node = node_submit($node);
			node_save($node);
			//db_query("INSERT INTO {pdf_generate} (nid, vid, pdffile) VALUES (%d, %d, '%s')", $node->nid, $node->vid, $file);
		}
		return 'Successful';
	}
